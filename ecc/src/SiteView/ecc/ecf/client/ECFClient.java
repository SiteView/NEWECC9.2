package SiteView.ecc.ecf.client;
...
public class ECFClient {
	public static final String GENERIC_SERVER_ID = "ecftcp://localhost:4280/";
	...
	{
		containers = ContainerFactory.getDefault().createContainer(GENERIC_CLIENT_CONTAINER);
	}

	private void addChannelToClients() throws Exception
	{
			IChannelContainerAdapter channelContainer = (IChannelContainerAdapter) containers
			        .getAdapter(IChannelContainerAdapter.class);
			channelContainer.createChannel(getNewID(CHANNEL_NAME), getChannelListener(containers.getID()), null);
	...
	private void connectClients(String s) throws Exception
	{
			connectClient(containers, IDFactory.getDefault().createStringID(s), null);
	}
	...
	{
		return (IChannelContainerAdapter) containers.getAdapter(IChannelContainerAdapter.class);
	}

	public void sendMessages(String s) throws IDCreateException, Exception
	{
		...
		toServer.sendMessage(s.getBytes());

		sleep(3000);

		handleEvent(containers.getID());
