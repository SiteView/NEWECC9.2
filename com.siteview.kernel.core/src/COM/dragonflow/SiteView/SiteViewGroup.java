...
public class SiteViewGroup extends MonitorGroup {
	...
	public SiteViewGroup() {
		...
		setProperty(pSiteViewLogfilePath, s + File.separator + "logs"
				+ File.separator + "SiteView.log");
	...
	public COM.dragonflow.Page.CGI.menus getNavItems(HTTPRequest httprequest) {
		...
		if (httprequest.actionAllowed("_browse")) {
			menus1.add(new COM.dragonflow.Page.CGI.menuItems("Browse",
					"browse", "", "page", "Browse Monitors"));
		}
		if (httprequest.actionAllowed("_preference")) {
			menus1.add(new COM.dragonflow.Page.CGI.menuItems("Remote UNIX",
					"machine", "", "page",
					"Add/Edit Remote UNIX/Linux profiles"));
			menus1.add(new COM.dragonflow.Page.CGI.menuItems("Remote NT",
					"ntmachine", "", "page",
					"Add/Edit Remote Win NT/2000 profiles"));
		}
		if (httprequest.actionAllowed("_tools")) {
			menus1.add(new COM.dragonflow.Page.CGI.menuItems("Tools",
					"monitor", "Tools", "operation",
					"Use monitor diagnostic tools"));
		}
		if (httprequest.actionAllowed("_progress")) {
			menus1.add(new COM.dragonflow.Page.CGI.menuItems("Progress",
					"Progress", "", "url", "View current monitoring progress"));
		}
		if (httprequest.actionAllowed("_browse")) {
			menus1.add(new COM.dragonflow.Page.CGI.menuItems("Summary",
					"monitorSummary", "", "page",
		...
		if (Platform.isPortal()) {
			...
			if (siteviewgroup.internalServerActive()) {
				if (siteviewgroup.dontStartGroups && !siteviewgroup.startingUp) {
					LogManager
							.log("RunMonitor",
									"Group file modified, will reload after initialization");
				} else {
	...
	public void startSiteView() {
		if (cCurrentSiteView != this) {
			...
			// "License Expired: only starting the limited HTTP server, no monitors are running.");
			...
			if (s.length() > 0) {
				...
				for (Enumeration enumeration = array.elements(); enumeration
						.hasMoreElements(); LogManager.log("RunMonitor", "   "
						+ enumeration.nextElement())) {
				}
				if (l == 0L) {
					LogManager.log("RunMonitor",
							"Startup script completed: exit (" + l + ")");
				} else {
					LogManager.log("RunMonitor",
							"STARTUP SCRIPT FAILED: exit (" + l + ")");
			...
			if (s1 != null) {
				...
				for (int i = 0; i < as.length; i++) {
					...
					try {
						...
						LogManager.log("Error",
								"Failed to initiate API listener: "
			...
			if (getSetting("_installTime").length() == 0) {
				...
				saveSettings();
				SiteViewMail.firstTimeMail(this, "SiteView Registration",
						"siteviewsales@merc-int.com");
			}
			boolean flag = (new File(Platform.getRoot() + File.separator
					+ "groups" + File.separator + "mirror.config")).exists();
			if (flag) {
				doHA();
			}
			...
			if (getSetting("_enterpriseImportServer").length() != 0) {
				...
				maintenanceScheduler.scheduleRepeatedPeriodicAction(
						new EnterpriseImportMail(s2, s3, s4), j);
			}
			if (getSetting("_autoDaily").length() != 0) {
				maintenanceScheduler.scheduleRepeatedDailyAction(
						new StatusMail(), "m,t,w,r,f,s,u", "07:07");
			}
			int k = getSettingAsLong("_fileCheckFrequency", 120) * 1000;
			int i1 = getSettingAsLong("_progressCheckFrequency", 120) * 1000;
			boolean flag1 = TextUtils.getValue(rtCfg,
					"_disableRepeatedSchedules").equalsIgnoreCase("true");
			// flag1 = true;
			if (!flag1) {
				maintenanceScheduler.scheduleRepeatedPeriodicAction(
						new DetectStateChange(), k);
			}
...
//			createMonitorCountLog();
			if (getSetting("_autoStartup").length() != 0) {
				if (!$assertionsDisabled
						...
						&& getMonitorCount() != totalPointsUsed) {
					throw new AssertionError(
							"point count is zero and monitor count non-zero; Monitor count: "
									+ getMonitorCount() + " Point count: "
									+ totalPointsUsed);
				}
				SiteViewMail.statusMail(this, "started");
			}
			progressGenerator = new GenerateProgress(new File(
					Platform.getRoot() + File.separator + "htdocs"
							+ File.separator + "Progress.html"), httprequest);
			progressGenerator.execute();
			maintenanceScheduler.scheduleRepeatedPeriodicAction(
					progressGenerator, i1);
			if (Platform.isUserAccessAllowed()) {
				GenerateProgress generateprogress = new GenerateProgress(
						new File(Platform.getRoot() + File.separator
								+ "userhtml" + File.separator + "Progress.html"),
						httprequest1);
			...
			if (s5.length() > 0) {
				LogManager.log("RunMonitor", "Running initialization URL: "
						+ s5);
			...
			if ((obj instanceof MonitorGroup) && !hashset.contains(obj)) {
				...
				for (SiteViewObject siteviewobject = monitorgroup.getParent(); siteviewobject != null;) {
					if (hashset1.contains(siteviewobject)) {
						...
						LogManager.log("Error",
								"Circular group hierarchy detected involving group "
										+ s);
						bHasCircularGroups = true;
						return true;
					}
					if ((MonitorGroup.class).isInstance(siteviewobject)) {
						siteviewobject = ((MonitorGroup) siteviewobject)
								.getParent();
					} else {
						siteviewobject = null;
					}
	...
	public void StartDDC() {
		...
		jdbcConfigScheduler.scheduleRepeatedPeriodicAction(
				JdbcConfig.getJdbcConfig(), i);
	...
	void startHTTPSServer() {
		...
		httpsServer = null;
		if (i != 0) {
			httpsServer = new HTTPServer(i, k, true, getSetting(
					"_httpKeepAliveEnabled").length() > 0);
			if (getSetting("_httpKeepAliveEnabled").length() > 0) {
				HTTPRequest.useHTTP11 = true;
			}
			httpsServer.addCGIVirtualDirectory("/SiteView/cgi",
					Platform.getRoot() + File.separator + "cgi");
			...
			while (!flag) {
				...
				if (s3.length() != 0) {
					LogManager.log("Error", "retry #" + j1 + " in " + l1 / 1000
							+ " seconds: could not start HTTP server on port "
							+ i + ", " + s7);
					Platform.sleep(l1);
				}
			}
			...
			if (!flag) {
				LogManager.log("Error", "Could not start HTTPS server on port "
						+ i + ", " + s7);
				...
				long l3 = (new File(Platform.getRoot() + "/ReadMe.htm"))
						.lastModified();
				String s8 = getProperty("_webserverAddress");
				String s9 = getProperty("_lockWebserverAddress");
				if (s9.length() == 0
						&& (!TextUtils.hasLetters(s8) || s8.equals("localhost"))) {
					boolean flag1 = false;
				...
				if (!s11.equals(s10) || l3 != l2) {
					LogManager.log("Error", "Updating URL in Readme.htm to "
							+ s11);
					UpdateConfig.updateFiles("https", "" + httpsServer.port,
							s8, getSetting("_adminRedirectFile"),
							getSetting("_userRedirectFile"));
					setProperty(
							"_readmeDate",
							""
									+ (new File(Platform.getRoot()
											+ "/ReadMe.htm")).lastModified());
				}
				saveSettings();
				if (httpsServer.port != j) {
					LogManager.log("Error", "Changing HTTP Server to port "
							+ httpsServer.port + ", " + s7);
	...
	void startHTTPServer() {
		...
		if (s3.length() > 0) {
			...
			try {
				s3 = Platform.getRoot() + File.separator + "logs"
						+ File.separator + "post.log";
				if (s4.length() > 0) {
					DailyFileLogger dailyfilelogger = new DailyFileLogger(s3,
							l, j1);
				...
				LogManager.log("Error", "Could not open POST request log file "
						+ s3);
			}
		}
		...
		httpServer = null;
		if (i != 0) {
			httpServer = new HTTPServer(i, k, false, getSetting(
					"_httpKeepAliveEnabled").length() > 0);
			httpServer.addCGIVirtualDirectory("/SiteView/cgi",
					Platform.getRoot() + File.separator + "cgi");
			...
			do {
				...
				if (s5.length() != 0) {
					LogManager.log("Error", "retry #" + l1 + " in " + k2 / 1000
							+ " seconds: could not start HTTP server on port "
							+ i + ", " + s9);
					Platform.sleep(k2);
				}
			...
			if (!flag) {
				LogManager.log("Error", "Could not start HTTP server on port "
						+ i + ", " + s9);
				...
				if (!s13.equals(s12) || l3 != l2) {
					LogManager.log("Error", "Updating URL in Readme.htm to "
							+ s13);
					UpdateConfig.updateFiles("http", "" + httpServer.port, s10,
							getSetting("_adminRedirectFile"),
							getSetting("_userRedirectFile"));
					setProperty(
							"_readmeDate",
							""
	...
	void logRunningMonitors() {
		...
		if (enumeration.hasMoreElements()) {
			...
			String s1;
			String s3;
			for (; enumeration.hasMoreElements(); LogManager.log("Error",
					Platform.productName
							+ " shutting down, monitor still running, " + s3
							+ ":" + s + ", " + s1)) {
	...
	public void stopSiteViewNoTimeout(int i, String s) {
		...
		if (i > 0) {
			LogManager.log("Error", "Waiting " + i
					+ " secs to allow monitors to complete.");
			Platform.sleep(i * 1000);
		} else {
			LogManager
					.log("Error",
							"Shutting down quickly, not allowing monitors to complete.");
		}
		logRunningMonitors();
		stopMonitor();
	...
	public static void killProcesses() {
		...
		if (as[0] == null) {
			LogManager.log("Errror",
					"Unsupported platform no kill command available");
			return;
		}
		...
		while (listiterator.hasNext()) {
			...
			try {
				...
				LogManager.log("Error",
						"Unable to kill potential zombie process.");
		...
		for (int i = 0; i < vector.size(); i++) {
			try {
				...
				LogManager
						.log("Error",
								"Failed to wait for the kill process during killing of zombie processes");
				interruptedexception.printStackTrace();
				// i ++;dingbing.xu
			}
		}
	...
	public static void writeMonitorStats() {
		...
		LogManager.log("RunMonitor",
				"Average Monitors Running: " + TextUtils.floatToString(f2, 2));
		LogManager.log("RunMonitor",
				"Peak Monitors Per Minute: " + TextUtils.floatToString(f, 2)
						+ " at " + TextUtils.prettyDate(l));
		LogManager.log("RunMonitor", "Peak Monitors Running: " + f1 + " at "
				+ TextUtils.prettyDate(l1));
		LogManager.log("RunMonitor", "Peak Monitors Waiting: " + i + " at "
				+ TextUtils.prettyDate(l2));
	}

	public static boolean shouldWriteHTML() {
		SiteViewGroup siteviewgroup = currentSiteView();
		return !siteviewgroup.internalServerActive()
				|| siteviewgroup.getSetting("_createStaticHTML").length() > 0;
	}
	...
	public static void saveMonitorPID(int i) {
		...
		try {
			...
			PrintWriter printwriter = FileUtils
	...
	public synchronized void registerSSEELogger(String s, int i, String s1,
			int j, String s2) throws Exception {
		try {
			if (sseeLogger != null) {
			...
			LogManager.log(
					"Error",
					"Could not register Frieda logger  exception: "
							+ exception.getMessage());
			throw exception;
		}
	}
	...
	public synchronized void registerToTopaz() {
		...
		if (topazSyncScheduler == null || !topazSyncScheduler.isRunning()) {
			try {
				...
				// LogManager.log("RunMonitor", TopazInfo.getTopazName() +
				// " Sync Scheduler Starting. ");
				// topazSyncScheduler.startScheduler(TopazInfo.getTopazName() +
				// " Sync Scheduler", true);
				// topazSyncScheduler.scheduleRepeatedPeriodicAction(TopazConfigurator.getTopazConfigurator(),
				// l);
			} catch (Exception exception) {
		...
		{
			try {
				LogManager.registerLogger("Topaz",
						new FileLogger(Platform.getRoot() + File.separator
								+ "logs" + File.separator + "topaz.log",
								TextUtils.toInt(getSetting("_maxAuxLogSize"))));
			} catch (Exception exception1) {
				// LogManager.log("Error", "Not able to register " +
				// TopazInfo.getTopazName() + " logger " +
		...
		// // LogManager.log("RunMonitor", "Could not register " +
		// TopazInfo.getTopazName() + " logger " + " exception: " +
	...
	public void saveSettings() {
		String s = groupsDirectory.getAbsolutePath() + File.separator
				+ "master.config";
		try {
			...
			LogManager.log("Error", "Could not write master.config file at:"
					+ s);
		}
	}
	...
	public Array getGroupIDs() {
		...
		if (!allGroupsLoaded) {
			...
			array = new Array();
			Monitor monitor;
			for (Enumeration enumeration = getMonitors(); enumeration
					.hasMoreElements(); array.add(monitor.getProperty("_id"))) {
	...
	private Enumeration addGroups(Array array) {
		...
		while (enumeration.hasMoreElements()) {
			...
			if (s.equals("master.config")) {
				...
				loadSettings();
				if (!s1.equals(getSetting("_httpPort"))
						&& getSetting("_httpPort").length() > 0) {
					startHTTPServer();
				}
				String s2 = getSetting("_httpSecurePort");
				if (!s2.equals(getSetting("_httpSecurePort"))
						&& getSetting("_httpSecurePort").length() > 0) {
					startHTTPSServer();
				}
...
//				createMonitorCountLog();
	...
	public void loadDynamic(HashMap hashmap) {
		VirtualMachine virtualmachine = VirtualMachine
				.createVirtualMachineObject(hashmap);
	...
	public void removeDynamic() {
		...
		for (Enumeration enumeration = getDynamics(); enumeration
				.hasMoreElements(); virtualmachine.getOwner().removeElement(
				virtualmachine)) {
	...
	void loadHistory(boolean flag) {
		...
		if (file.exists()) {
			try {
				...
				while (enumeration.hasMoreElements()) {
					HashMap hashmap = (HashMap) enumeration.nextElement();
					HistoryReport historyreport = HistoryReport
	...
	void removeHistory() {
		...
		for (Enumeration enumeration = getReports(); enumeration
				.hasMoreElements(); historyreport.getOwner().removeElement(
				historyreport)) {
	...
	public static void updateStaticPages(String s) {
		...
		if (siteviewgroup.internalServerActive()) {
			...
			monitorgroup = MonitorGroup.loadGroup(I18N.toNullEncoding(s),
					Platform.getRoot() + File.separator + "groups"
							+ File.separator + s + ".mg");
		}
	...
	public String htmlPath(HTTPRequest httprequest) {
		return Platform.getRoot() + "/" + httprequest.getAccountDirectory()
				+ "/SiteView.html";
	}
	...
	public void writeAllHTML() {
		...
		if (shouldWriteHTML() && Platform.isUserAccessAllowed()) {
			...
			try {
				PrintWriter printwriter = FileUtils
						.MakeOutputWriter(new FileOutputStream(s));
				try {
					HTTPRequest httprequest = new HTTPRequest();
					...
					printwriter
							.println("<hr>There was an unexpected problem:<br>"
	...
	public void printSpecial(String s, PrintWriter printwriter,
		...
		} else if (s.indexOf("Reports.html") != -1
				&& s.indexOf("/Detail") == -1) {
			reportPage reportpage = new reportPage();
			reportpage.request = httprequest;
			reportpage.printBody(printwriter);
		...
		} else if (s.indexOf("Progress.html") != -1
				&& s.indexOf("/Detail") == -1) {
			if (!httprequest.actionAllowed("_progress")) {
	...
	public Array getTopLevelMonitors(HTTPRequest httprequest) {
		...
		while (enumeration.hasMoreElements()) {
			...
			if (array1.size() != 0 ? CGI.allowedByGroupFilter(
					I18N.toDefaultEncoding(monitor.getProperty(pID)), array1)
					&& !isParentInFilter(array1, parent) : parent.length() == 0
					&& health.length() == 0) {
				array.add(monitor);
			}
		}
	...
	private String createNewTableRow(int i) {
		return "</tr>\n\n<tr><td><img src=/SiteView/htdocs/artwork/empty51.gif height="
				+ i + " width=1></td></tr><tr>";
	}

	private int printSiteSeerGauge(StringBuffer stringbuffer,
			HTTPRequest httprequest, boolean flag, int i, int j, String s) {
		if (!flag) {
			String s1 = getSetting("_siteseerAccount");
			if (s1.length() > 0
					&& getSetting("_siteseerDisabled").length() == 0) {
				if (i % j == 0) {
					stringbuffer.append(createNewTableRow(35));
				}
				String s2 = overviewPage.fetchSiteSeerCategory(MasterConfig
						.getMasterConfig());
				String s4 = "";
				if (getSetting("_siteseerLoginInURL").length() > 0) {
					String s5 = URLEncoder
							.encode(getSetting("_siteseerUsername"));
					String s7 = URLEncoder
							.encode(getSetting("_siteseerPassword"));
					if (s.equals("user")) {
						String s9 = URLEncoder
								.encode(getSetting("_siteseerReadOnlyUsername"));
						if (s9.length() > 0) {
							s5 = s9;
							s7 = URLEncoder
									.encode(getSetting("_siteseerReadOnlyPassword"));
						}
					}
				...
				String s6 = "http://" + s4 + getSetting("_siteseerHost")
						+ "/SiteView/accounts/" + s1
						+ "/htdocs/SiteView.html?account=" + s1;
				String s8 = getSetting("_siteseerTitle");
				String s10 = "0";
				if (displayGauges) {
					stringbuffer
							.append("<td valign=top><table cellpadding=1 border=0 cellspacing=0 width=95>\n<tr><td valign=top colspan=2><a href="
									+ s6
									+ "><img border=0 src="
									+ getGaugeArt(s10)
									+ " alt="
									...
									+ "<td valign=top width=75><a href="
									+ s6
									+ " title=\""
									+ s8
									+ "\" style=\"color:#33AAFF\"><b>"
									+ s8
									+ "</b></a></td></tr></table></td>\n");
				} else {
					stringbuffer
				...
				stringbuffer
						.append("<td valign=top><table cellpadding=1 border=0 cellspacing=0 width=128>\n<tr><td valign=top><img src=/SiteView/htdocs/artwork/empty201.gif height=20 width=1></td>\n<td align=left valign=top width=96><a href=/SiteView/cgi/go.exe/SiteView?page=siteseerPrefs"
								...
								+ ">"
								+ "<img src=/SiteView/htdocs/artwork/add_SSr.gif height=17 width=97 border=0></a></td></tr></table></td>\n\n");
				i++;
			}
		}
		return i;
	...
	private int printCreateGroupGauge(StringBuffer stringbuffer,
			HTTPRequest httprequest, int i) {
		String s = "";
		Array array = CGI.getGroupFilterForAccount(httprequest);
		if (httprequest.actionAllowed("_groupEdit") && array.size() == 0) {
			s = "<A HREF=/SiteView/cgi/go.exe/SiteView?page=group&operation=Add&account="
					+ httprequest.getAccount()
					+ "><img src=/SiteView/htdocs/artwork/create_group.gif height=17 width=97 border=0></A>";
		}
		stringbuffer
	...
	public void printPage(PrintWriter printwriter, HTTPRequest httprequest)
			throws Exception {
		...
		displayGauges = getSetting("_displayGauges")
				.equalsIgnoreCase("CHECKED");
		printwriter.println("<HTML>");
		CGI.printHeadTag(printwriter, Platform.productName,
				refreshTag(k, "SiteView.html"));
		...
		if (flag1) {
			printwriter
					.println("<TABLE border=0 cellpadding=0 cellspacing=0 width=600>\n<TR><TD>");
			CGI.printButtonBar(printwriter, "GetStart.htm#viewer", "SiteView",
					httprequest, MasterConfig.getMasterConfig(), menus1, flag1);
		...
		printwriter
				.println("<table border=0 cellspacing=0 width=600><tr><td valign=top colspan=4>\n");
		printwriter
				.println("<tr><td colspan=4><img align=left src=/SiteView/htdocs/artwork/empty51.gif height=40 width=75 border></td></tr>\n");
		printwriter.println("</table>\n");
		printwriter.println("<table border=0 cellspacing=0 width=600><tr>\n");
		StringBuffer stringbuffer = new StringBuffer();
		stringbuffer
				.append("<table align=center bgcolor=#000000 width=350><tr><td></td>");
		int l = 0;
		...
		if (array.size() > 0) {
			...
			for (Enumeration enumeration = array.elements(); enumeration
				...
				l++;
				MonitorGroup monitorgroup2 = (MonitorGroup) enumeration
						.nextElement();
				String s8 = fixNameLengths(monitorgroup2);
				s8 = s8.trim();
				String s10 = monitorgroup2.getProperty(pName);
				s10 = s10.trim();
				String s12 = I18N.toDefaultEncoding(monitorgroup2
						.getProperty(pID));
				...
				if (displayGauges) {
					stringbuffer
							.append("<td valign=top><table cellpadding=1 border=0 cellspacing=0 width=128>\n<tr><td valign=top colspan=2><a href="
									...
									+ s17
									+ ">"
									+ monitorgroup2.progressArt()
									+ "<img border=0 src="
									+ getCategoryArt(s15)
									+ " alt="
									+ s15
									+ " height=21 width=20></a></td>\n"
									+ "<td valign=top width=105><a href="
									+ s17
									+ " title=\""
									+ s10
									+ "\""
			...
			if (array.size() == 1 && s4.indexOf("Examples") != -1) {
				stringbuffer.append(createNewTableRow(35));
				stringbuffer
						.append("<td valign=top><table cellpadding=5 border=0 cellspacing=5 width=460>\n<tr><td valign=top><p style=\"color:#33AAFF\"><b>Only the \""
								+ s4.toString()
								+ "\" group is currently defined.</b><br><br> "
								+ " To get started:<br>"
								+ " 1. Click the <b>"
								+ s4.toString()
								+ "</b> group name to view the subgroups and monitors in this group<br>"
								+ " 2. Click the <b>SiteView</b> button to return to this view<br>"
								+ " 3. Click the <b>Create Group</b> link to create more groups<br>"
								+ " 4. Add monitors or subgroups to the new groups<br>"
								+ "</td></tr>\n" + "</table></td>\n");
			}
		} else {
			stringbuffer.append(createNewTableRow(35));
			...
			// + TopazInfo.getTopazName() + " Configuration Management.<br><br>"
			// + " * Please use " + TopazInfo.getTopazName() +
			// " Configuration Console to add new monitors or subgroups.<br>" +
			// "</td></tr>\n" + "</table></td>\n");
			// }
		}
		int i = l;
		...
		if (httprequest.actionAllowed("_alertEdit")
				&& httprequest.actionAllowed("_alertList")
				&& getSetting("_alertIconLink").length() > 0) {
			...
			s6 = "<td valign=\"middle\" align=\"right\"><p style=\"color: #a5a5a5; font-size:8pt; font-weight: bold; font-family: 'Arial', 'Helvetica', sans-serif;\">Global Alerts: </td>"
					+ printAlertIconLink(httprequest, "_master", "_config",
							array2);
		}
		stringbuffer1
		...
		String s11 = "";
		if (Platform.isDemo()) {
			s11 = "<TR><TD><p class=license align=center><a href=/SiteView/cgi/go.exe/SiteView?page=demo&account="
					+ httprequest.getAccount()
					+ ">"
					+ LocaleUtils.getResourceBundle().getString("ResetDemo")
					+ "</a></p></td></tr>\n";
			...
			s11 = "\n<TR><TD><p class=license align=center>"
					+ LUtils.getLicenseSummary() + "</p></TD></TR>\n";
		...
		String s14 = "";
		if (httprequest.actionAllowed("_progress")) {
			s14 = "<TR><TD><p class=progress align=center><b><a href=Progress.html?hideState=true>"
					+ progressSummary + "</a></b></p></td></tr>\n";
		}
		printwriter
				.print("<img src=/SiteView/htdocs/artwork/empty651.gif height=45 width=1><TABLE class=layout align=center border=0 cellpadding=12 cellspacing=0 width=600>"
						+ s9
						+ s11
						+ s14
						+ "<TR><TD><p class=fine align=CENTER>"
						+ "<font size=-1>"
						+ Platform.refreshLabel(k)
						+ LocaleUtils.getResourceBundle().getString(
								"LastRefresh")
						+ " "
						+ s7
						+ "</font></p></td></tr>\n");
		printwriter.print("<TR><TD><p class=fine align=center>"
				+ LocaleUtils.getResourceBundle().getString(
						"FindOutWhatsInThisRelease")
				+ " <a href=\"/SiteView/ReleaseNotes.htm\" TARGET=Help>"
				+ LocaleUtils.getResourceBundle().getString(
						"GoToTheReleaseNotes") + "</a></p></td></tr>\n");
	...
	public String licenseParagraph(HTTPRequest httprequest) {
		...
		if (httprequest.getAccount().equals("administrator")) {
			...
			if (i == 0 || i == 1) {
				String s1 = LUtils.getLicenseSummary(
						MasterConfig.getMasterConfig(), true);
				s = "<hr><CENTER>"
						+ s1
						+ "<br>Order <a href="
						+ Platform.homeURLPrefix
						+ "/OrderOpts.htm>Online</a> or send e-mail to <a href=mailto:"
						+ Platform.salesEmail + ">" + Platform.salesEmail
						+ "</a></a>" + "</CENTER><hr>";
			}
		}
		return s;
	...
	public int getAIMsCount() {
		...
		for (Enumeration enumeration = getMonitors(); enumeration
			...
			while (enumeration1.hasMoreElements()) {
				...
				if (!(monitor instanceof SubGroup)
						&& monitor.getClassPropertyString("classType").equals(
								"application")) {
					i++;
				}
			}
	...
	public void notifyDispatcherAtStartup() {
		for (Enumeration enumeration = getMonitors(); enumeration
			...
			while (enumeration1.hasMoreElements()) {
				Monitor monitor = (Monitor) enumeration1.nextElement();
				if (!(monitor instanceof SubGroup) && !monitor.isDisabled()
						&& (monitor instanceof DispatcherMonitor)
						&& ((DispatcherMonitor) monitor).isDispatcher()) {
					if (getDispatcherMonitorInstanceCount(monitor) == 1) {
						DispatcherMonitor.notifyDispatcherMonitor(
								DispatcherMonitor.INIT_MON,
								(DispatcherMonitor) monitor);
					}
					DispatcherMonitor.notifyDispatcherMonitor(
							DispatcherMonitor.INIT_CONN,
							(DispatcherMonitor) monitor);
				}
			}
		}
	...
	private void checkDispatcherForClear(AtomicMonitor atomicmonitor) {
		...
		DispatcherMonitor.notifyDispatcherMonitor(
				DispatcherMonitor.TERMINATE_CONN, atomicmonitor);
		if (getDispatcherMonitorInstanceCount(atomicmonitor) - 1 <= 0) {
			DispatcherMonitor.notifyDispatcherMonitor(
					DispatcherMonitor.TERMINATE_MON, atomicmonitor);
		}
	...
	public void checkDispatcherForStart(AtomicMonitor atomicmonitor) {
		...
		if (getDispatcherMonitorInstanceCount(atomicmonitor) == 0) {
			DispatcherMonitor.notifyDispatcherMonitor(
					DispatcherMonitor.INIT_MON, atomicmonitor);
		}
	}
		...
		for (Enumeration enumeration = getMonitors(); enumeration
			...
			while (enumeration1.hasMoreElements()) {
				...
				if (!(monitor1 instanceof SubGroup)
						&& (monitor1 instanceof DispatcherMonitor)
						&& ((DispatcherMonitor) monitor1).isDispatcher()
						&& !monitor1.isDisabled()
						&& monitor1.getProperty("_class").equals(
								monitor.getProperty("_class"))) {
					i++;
				}
			}
		}
		...
		for (Enumeration enumeration = getMonitors(); enumeration
			...
			while (enumeration1.hasMoreElements()) {
				...
				if (!(monitor instanceof SubGroup)
						...
						&& ((AtomicMonitor) monitor).isDispatcher()
						&& !monitor.isDisabled()) {
					i++;
				}
			}
	...
	public int checkAuthorization(HTTPRequest httprequest)
		...
		if (Platform.isPortal() && httprequest.getPortalServer().length() > 0) {
			PortalSiteView portalsiteview = (PortalSiteView) Portal
					.getSiteViewForServerID(httprequest.getPortalServer());
			if (portalsiteview == null) {
				throw new HTTPRequestException(557);
			}
		...
		if (s.equals("/SiteView/docs/UGtoc.htm")
				|| s.equals("/SiteView/README.htm")
				|| s.equals("/SiteView/ReleaseNotes.htm")
				|| s.equals("/SiteView/docs/UserGuide.htm")) {
			return 200;
		}
	...
	private void doHA() {
		...
		try {
			LogManager.registerLogger(
					"Mirror",
					new FileLogger(Platform.getRoot() + File.separator + "logs"
							+ File.separator + "mirror.log", TextUtils
							.toInt(getSetting("_maxAuxLogSize"))));
		} catch (Exception exception) {
		}
	...
	public int createMonitorCountLog() {
		...
		String s2 = Platform.getRoot() + File.separator + "logs"
				+ File.separator + s;
		PrintWriter printwriter = null;
		try {
			if (s2 != null) {
				printwriter = FileUtils.MakeOutputWriter(new FileOutputStream(
						s2));
				...
				if (s7 != null && s7.length() > 0) {
					...
					for (int l2 = 0; l2 < array1.size(); l2++) {
						...
						printwriter.println(s9 + ": " + s11 + " Monitors, "
								+ s13 + " Points");
						s6 = s6 + s9.charAt(0);
						s6 = s6 + s11;
					}
					...
					for (Enumeration enumeration2 = hashtable.elements(); enumeration1
							.hasMoreElements()
							&& enumeration2.hasMoreElements();) {
						String s8 = (String) enumeration1.nextElement();
						int i3 = ((Integer) enumeration2.nextElement())
								.intValue();
						...
						printwriter.println(s8 + ": " + s10 + " Monitors, "
								+ s12 + " Points");
						s6 = s6 + s8.charAt(0);
						s6 = s6 + s10;
					}
			...
			LogManager.log("Error",
					"Error writing " + s2 + ": " + ioexception.getMessage());
			ioexception.printStackTrace();
		} finally {
			if (printwriter != null) {
	...
	private Array sortClassList(Array array) {
		...
		while (enumeration.hasMoreElements()) {
			...
			try {
				AtomicMonitor atomicmonitor = (AtomicMonitor) class1
						.newInstance();
	...
	public static HashMap loadRTConfig(HashMap hashmap) {
		...
		if (Platform.customConfigPath.length() > 0) {
			...
			for (HashMapIterator hashmapiterator = hashmap1.begin(); !hashmapiterator
	...
	public static void getGroupsMonitors(Vector vector, Vector vector1,
			boolean flag) {
	...
	static {
		...
		pAdminURL = new StringProperty("_adminURL",
				"http://localhost:8888/SiteView");
		StringProperty astringproperty[] = { pWebserverAddress, pAdminURL };
		addProperties("SiteViewGroup", astringproperty);
		HashMap hashmap = MasterConfig.getMasterConfig();
		if (hashmap.get("_killProcesses") != null
				&& ((String) hashmap.get("_killProcesses")).length() > 0) {
