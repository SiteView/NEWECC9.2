...
public abstract class CGI {
	public static class menuItems {
		...
		public menuItems(String s, String s1, String s2, String s3, String s4) {
			menuLabel = "";
			menuLink = "";
			opLink = "";
			linkClass = "";
	...
	jgl.Array _getUsedMonitorClasses() {
		...
		while (enumeration.hasMoreElements()) {
			MonitorGroup monitorgroup = SiteViewGroup.currentSiteView()
					.getGroup(
	...
	public static void printFooter(java.io.PrintWriter printwriter,
		...
		if (httprequest == null
				|| !Platform.isSiteSeerAccount(httprequest.getAccount())) {
			String s = "";
	...
	public void printButtonBar(String s, String s1, menus menus1) {
	...
	public static String reportURL(COM.dragonflow.HTTP.HTTPRequest httprequest) {
		if (httprequest.getAccount().equals("user")) {
			return Platform.getURLPath("userhtml", httprequest.getAccount())
					+ "/Reports.html";
		} else {
	...
	public static void printButtonBar(java.io.PrintWriter printwriter,
			String s, String s1, COM.dragonflow.HTTP.HTTPRequest httprequest,
			jgl.HashMap hashmap) {
	...
	public static void printButtonBar(java.io.PrintWriter printwriter,
			String s, String s1, COM.dragonflow.HTTP.HTTPRequest httprequest,
			jgl.HashMap hashmap, boolean flag) {
	...
	public static void printButtonBar(java.io.PrintWriter printwriter,
			String s, String s1, COM.dragonflow.HTTP.HTTPRequest httprequest,
			jgl.HashMap hashmap, menus menus1, boolean flag) {
		if (Platform.isPortal()) {
			...
			String s5 = Portal.getViewContent(s3, httprequest);
		...
		if (!s1.equals("SiteView")) {
			...
			printwriter.println(Platform.licenseHeader(hashmap, true,
					httprequest.getAccount()));
		}
		printwriter
	...
	public static void printHeadTag(java.io.PrintWriter printwriter, String s,
			String s1) {
	...
	public static void printHeadTag(java.io.PrintWriter printwriter, String s,
		...
		String s3 = SiteViewGroup.getServerID();
	...
	public static String getGroupDetailURL(
		...
		return "/SiteView/" + httprequest.getAccountDirectory() + "/Detail"
				+ COM.dragonflow.HTTP.HTTPRequest.encodeString(s) + ".html"
				+ s1;
	}
	...
	public void printRefreshHeader(String s, String s1, int i) {
		s1 = getWhereURL(s1);
	...
	public void printLicenseExceeded(Monitor monitor, String s, int i, String s1) {
		...
		if (s1.equals("license")) {
			String s3 = COM.dragonflow.Utils.LUtils.getLicenseExceededHTML();
	...
	public static boolean isRelated(String s, String s1) {
		...
		while (s.length() != 0) {
			...
			if (COM.dragonflow.Page.treeControl.useTree()) {
				...
				monitor = (Monitor) siteviewgroup.getElement(as[0]);
			} else {
				monitor = (Monitor) siteviewgroup.getElement(s);
			}
			if (monitor == null) {
				break;
			}
	...
	public static jgl.Array getAllowedGroupIDsForAccount(
		...
		if (COM.dragonflow.Page.CGI.isPortalServerRequest(httprequest)) {
			...
			for (int i = 0; i < array.size(); i++) {
				MonitorGroup monitorgroup = (MonitorGroup) array.at(i);
				array2.add(COM.dragonflow.Utils.I18N
			...
			jgl.Array array1 = COM.dragonflow.Utils.I18N
					.toDefaultArray(SiteViewGroup.currentSiteView()
							.getGroupIDs());
	...
	public static jgl.Array getGroupFilterForAccount(
		...
		if (Platform.isStandardAccount(s)) {
			Enumeration enumeration = httprequest.getUser().getMultipleValues(
					"_group");
	...
	public static boolean allowedByGroupFilter(String s, jgl.Array array) {
	...
	public static boolean isGroupAllowedForAccount(String s, jgl.Array array) {
	...
	public static String getGroupFullLinks(Monitor monitor) {
	...
	public static String getGroupPath(Monitor monitor, String s, boolean flag) {
	...
	public static String getGroupPath(Monitor monitor, String s, boolean flag,
		...
		while (true) {
			...
			String s3 = COM.dragonflow.Page.CGI.getGroupName(monitor, s);
			...
			monitor = (Monitor) siteviewobject.getElement(s);
		}
		return s2;
	}

	public static String getGroupName(Monitor monitor, String s) {
		COM.dragonflow.Utils.I18N.test(s, 0);
		String s1;
		if (monitor == null) {
	...
	public static String getGroupName(jgl.Array array, String s) {
		COM.dragonflow.Utils.I18N.test(s, 0);
		if (array.size() > 0) {
	...
	public static String getGroupName(jgl.HashMap hashmap, String s) {
		String s1 = null;
		COM.dragonflow.Utils.I18N.test(s, 0);
		if (Portal.isPortalID(s)) {
			...
			if (monitorgroup != null) {
				PortalSiteView portalsiteview = (PortalSiteView) monitorgroup
						.getOwner();
				if (portalsiteview != null) {
					s1 = monitorgroup.getProperty(MonitorGroup.pName) + " on "
	...
	public static String getGroupIDFull(String s, String s1) {
		if (s1.length() > 0 && Portal.isPortalID(s1)) {
			s = Portal.makePortalID(Portal.getServerID(s1), s, "");
		}
		return s;
	}

	public static String getGroupIDFull(String s, SiteViewObject siteviewobject) {
		if (siteviewobject instanceof PortalSiteView) {
			s = Portal.makePortalID(
					siteviewobject.getProperty(PortalSiteView.pID), s, "");
		}
		return s;
	}

	public String getGroupIDFull(String s) {
		return COM.dragonflow.Page.CGI.getGroupIDFull(s,
				request.getPortalServer());
	}
	...
	public SiteViewObject getSiteView() {
		return Portal.getSiteViewForID(request.getPortalServer());
	}
	...
	public static String computeGroupName(String s,
		...
		while (enumeration.hasMoreElements()) {
			...
			s3 = s3.toUpperCase() + '.';
			if (s3.startsWith(s2)) {
				int j = COM.dragonflow.Utils.TextUtils.readInteger(s3,
						s2.length());
				if (j < 0) {
					j = 0;
				}
	...
	public static String getGroupFilePath(String s,
		...
		String s2 = "";
		if (httprequest != null) {
			s2 = Portal.cleanPortalServerID(httprequest.getPortalServer());
		}
		if (Portal.isPortalID(s)) {
		...
		String s4 = "";
		if (s2.length() > 0) {
			s4 = Portal.getPortalSiteViewRootPath(s2) + s3;
		} else {
	...
	public jgl.Array ReadGroupFrames(String s) throws java.io.IOException {
	...
	public static Enumeration getValues(jgl.HashMap hashmap, String s) {
	...
	public static jgl.HashMap loadMasterConfig(
		...
		String s = COM.dragonflow.Page.CGI.getGroupFilePath("_master",httprequest);
		jgl.Array array;
		try {
...
//				BufferedReader input = new BufferedReader(new FileReader(masterfile));
//				String st = "";
//				String s1 = "";
...
//				while(ien.MoveNext()){
//					BusinessObject machines=(BusinessObject) ien.get_Current();
//					ICollection ico_1=machines.get_FieldNames();
//					IEnumerator ien_1=ico_1.GetEnumerator();
//					if (machines.GetField("RemoteMachineType").get_NativeValue().toString()
//							.equals("RemoteUnix")) {
//						s1 += "_remoteMachine=";
//
//					} else if (machines.GetField("RemoteMachineType").get_NativeValue()
//							.toString().equals("RemoteNT")) {
//						s1 += "_remoteNTMachine=_os=NT _sshClient=java _sshConnectionsLimit=3 _sshAuthMethod=password ";
//					} else if (machines.GetField("RemoteMachineType").get_NativeValue()
//							.toString().equals("RemoteEquipment")) {
//
//					} else {
//						s1 += "_snmp=";
//					}
//					while(ien_1.MoveNext()){
//						String columName=(String) ien_1.get_Current();
//						if(columName.equals("RemoteMachineType")){
//							continue;
//						}
//						String datavalue=machines.GetField("").get_NativeValue().toString();
...
//								if (parmName.equals("_trace")
//										|| parmName.equals("_disableCache")
//										|| parmName.equals("_version2")) {
...
//								}
//							}
//							s1 += parmName + "=" + datavalue + " ";
//						}
//					}
//				}
//				String queryUnixSql = "select * from RemoteMachine";
...
//			}
	...
	void saveSettings(jgl.HashMap hashmap) throws java.io.IOException {
		if (request.isStandardAccount()) {
			...
			SiteViewGroup.updateStaticPages(request.getAccount());
		}
	}

	String getMonitorOptionsHTML(String s, String s1, int i)
			throws java.lang.Exception {
		jgl.Array array = new Array();
		array.add(s);
		jgl.Array array1 = new Array();
		...
		for (Enumeration enumeration = array3.elements(); enumeration
			...
			while (enumeration1.hasMoreElements()) {
				MonitorGroup monitorgroup = (MonitorGroup) enumeration1
						.nextElement();
				String s2 = monitorgroup.getProperty(Monitor.pID);
				...
				if ((i & 2) != 0) {
					...
					if (monitorgroup.getTreeSetting("_truncateGroupName")
						...
						String s8 = s1.length() <= i1 ? s1 : s1.substring(i1);
				...
				while (enumeration2.hasMoreElements()) {
					...
					if (!(monitor instanceof SubGroup) && (i & 1) != 0) {
						String s4 = "";
						String s6 = monitor.getProperty(Monitor.pID);
						String s7;
						if ((i & 0x20) != 0) {
							s7 = s6 + " " + s2;
						} else {
	...
	public String getSelectedOptionsHTML(jgl.Array array, String s, String s1,
			jgl.Array array1, jgl.Array array2, int i)
			throws java.lang.Exception {
		StringBuffer stringbuffer = new StringBuffer();
	...
	public static String getEndTimeHTML(long l) {
		java.util.Date date = Platform.makeDate();
		l *= 1000L;
		return COM.dragonflow.Page.CGI.getTimeHTML(
				new Date(date.getTime() + l), "end");
	}

	public static String getTimeHTML(java.util.Date date, String s) {
		StringBuffer stringbuffer = new StringBuffer();
		...
		stringbuffer
				.append(COM.dragonflow.Utils.TextUtils.prettyDateDate(date));
		stringbuffer.append("\"> ");
		return stringbuffer.toString();
	}

	public static String getOptionsHTML(jgl.Array array, String s) {
	...
	public static String getOptionsHTML(java.util.Vector vector, String s) {
		jgl.HashMap hashmap = new HashMap();
		if (s.length() > 0) {
			hashmap.put(s, "true");
		}
	...
	public static void expandSubgroupIDs(String s, jgl.Array array,
			COM.dragonflow.Page.CGI cgi) {
		COM.dragonflow.Utils.I18N.test(s, 0);
		java.lang.Object obj = SiteViewGroup.currentSiteView();
		if (cgi != null) {
			obj = cgi.getSiteView();
		}
		array.add(s);
		...
		if (monitorgroup != null) {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (monitor instanceof SubGroup) {
					COM.dragonflow.Page.CGI.expandSubgroupIDs(
							COM.dragonflow.Utils.I18N.toDefaultEncoding(monitor
									.getProperty(SubGroup.pGroup)), array, cgi);
				}
			}
		}
	}
	...
	public String getPageLink(String s, String s1, String s2) {
		String s3 = "/SiteView/cgi/go.exe/SiteView?page=" + s + "&account="
				+ request.getAccount();
		...
		String s5 = COM.dragonflow.Utils.I18N.toDefaultEncoding(request
				.getValue("groupFilter"));
		if (s5.length() > 0) {
			s3 = s3 + "&groupFilter="
	...
	public String getPagePOST(String s, String s1, String s2) {
	...
	public String getPageGET(String s, String s1, String s2) {
		return getPageForm(s, s1, "GET", s2);
	}

	public String getPageForm(String s, String s1, String s2) {
		return getPageForm(s, s1, s2, "");
	}

	public String getPageForm(String s, String s1, String s2, String s3) {
		String s4 = "<FORM ACTION=/SiteView/cgi/go.exe/SiteView method=" + s2
	...
	void saveMachines(jgl.Array array, String s) throws java.io.IOException {
		jgl.HashMap hashmap = getMasterConfig();
		hashmap.remove(s);
		for (int i = 0; i < array.size(); i++) {
...
//				System.out.println("ssssssssssssss");
	...
	protected static String getComboOptionHTML(String s, boolean flag) {
		StringBuffer stringbuffer = new StringBuffer();
		stringbuffer.append("<option ");
		if (flag) {
			stringbuffer.append("SELECTED");
	...
	public int platformOS() {
		...
		if (isPortalServerRequest()) {
			PortalSiteView portalsiteview = (PortalSiteView) Portal
					.getSiteViewForServerID(Portal.cleanPortalServerID(request
							.getPortalServer()));
			if (portalsiteview != null) {
	...
	private static void printNavBarMessages(java.io.PrintWriter printwriter) {
		...
		if (COM.dragonflow.Utils.TextUtils
				.getValue(hashmap, "_suspendMonitors").equals("CHECKED")) {
			printwriter
		...
		if (SiteViewGroup.currentSiteView().hasCircularGroups()
				&& SiteViewGroup.currentSiteView().checkForCircularGroups()) {
			printwriter
