...
public class PropertiedObject {
	...
	protected static void addProperties(String s,
		...
		PropertyTable propertytable = (PropertyTable) cPropertyMap.get(class2
				.getName());
	...
	public static void addPropertyToPropertyMap(String s,
	...
	public void printValuesTable() {
		if (valuesTable != null) {
			for (HashMapIterator hashmapiterator = valuesTable.begin(); !hashmapiterator
					.atEnd(); hashmapiterator.advance()) {
				System.out.println(hashmapiterator.key() + "="
	...
	public HashMap getTemplateTable() {
		if (templateID != null
				&& templateTableLastUpdate < templateCacheLastUpdate) {
			templateTable = null;
					...
					for (int i = 0; i < array.size(); i++)
						try {
							...
							for (int j = 1; j < array1.size(); j++) {
								...
								hashmap2.put(
										TextUtils.getValue(hashmap3, "__id"),
										hashmap3);
							}
	...
	static void resolveTemplateReferences(HashMap hashmap) {
		if (hashmap != null) {
			...
			for (Enumeration enumeration = hashmap.keys(); enumeration
					.hasMoreElements(); resolveTemplateReferences(hashmap,
					hashmap1, 0, hashmap.size()))
	...
	static void resolveTemplateReferences(HashMap hashmap, HashMap hashmap1,
			int i, int j) {
		if (i > j) {
			LogManager.log("Error", "Loop detected in templates.config");
			return;
		}
		for (Enumeration enumeration = hashmap1.values("__template"); enumeration
				.hasMoreElements();) {
	...
	public static Vector getTemplateList(String s) {
		...
		if (s.length() > 0) {
			...
			s1 = Portal.getPortalSiteViewRootPath(s) + "/groups/"
					+ TEMPLATES_FILE;
		}
		try {
	 ...
	 * @param flag
	 */
	public void initializeValuesTable(HashMap hashmap, HashMap hashmap1,
			boolean flag) {
		for (HashMapIterator hashmapiterator = hashmap.begin(); !hashmapiterator
			...
			if (stringproperty != null) {
				...
				if (obj instanceof Array) {
					for (Enumeration enumeration = ((Array) obj).elements(); enumeration
							.hasMoreElements(); hashmap1.add(stringproperty,
	...
	private HashMap findTable(StringProperty stringproperty) {
		if (getTemplateTable() != null
				&& templateTable.get(stringproperty) != null) {
			return templateTable;
		}
		if (stringproperty != null && valuesTable != null
				&& valuesTable.get(stringproperty) != null) {
			return valuesTable;
		} else {
			return null;
		}
	...
	public Array getProperties(StringProperty stringproperty)
			throws NullPointerException {
	...
	public synchronized Enumeration getMultipleValues(
			StringProperty stringproperty) {
		jgl.HashMap hashmap = findTable(stringproperty);
		if (hashmap != null) {
	...
	public static String format(BusinessObject machines) {
		String password;
		...
		if (machines.GetField("RemoteMachineType").get_NativeValue().toString()
			...
			remoteMachineInfo = "_remoteNTMachine=_status=unknow _os=NT _sshClient=java _sshConnectionsLimit=3 _sshAuthMethod=password ";
		} else if (machines.GetField("RemoteMachineType").get_NativeValue()
		...
		ICollection ico = machines.get_FieldNames();
		IEnumerator ien = ico.GetEnumerator();
		while (ien.MoveNext()) {
			String columName = (String) ien.get_Current();
			if(columName.equals("ConnectionLimit")){
				continue;
			}
			String datavalue = machines.GetField(columName).get_NativeValue().toString();
	...
	public void setPropertyWithObject(StringProperty stringproperty,
	...
	public void addProperty(StringProperty stringproperty, String s)
			throws NullPointerException {
		if (valuesTable == null) {
			setProperty(stringproperty, s);
		} else {
	...
	public void incrementProperty(StringProperty stringproperty) {
		setProperty(stringproperty,
				TextUtils.increment(getProperty(stringproperty)));
	}
